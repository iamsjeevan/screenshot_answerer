...
def escape_markdown_v2(text: str) -> str:
    ...
    # The escape character '\' must be escaped first.
    text = text.replace('\\', '\\\\')
...
def get_ai_answer(input_text: str) -> str:
    ...
    prompt = f"Based on the following text, provide a Python code solution. ONLY the raw Python code think for longer time and give the best solution possible it is okay even if it takes longer time. No comments or explanation.\n\nText:\n---\n{input_text}\n---"
    try:
...
async def send_telegram_message_async(message_text: str):
    ...
    try:
        ...
        logger.info(f"Telegram status message sent successfully!")
    except Exception as e:
...
async def send_code_as_file_async(code_text: str, caption: str):
    """
    ASYNC function to send a string of code as a .py file attachment via Telegram.
    This is the robust way to handle large code blocks and avoid formatting issues.
    """
    if not telegram_bot or not TELEGRAM_CHAT_ID:
        logger.error("Telegram Bot not initialized or Chat ID missing. Cannot send file.")
        return
...
def perform_ocr_task():
    """
    Captures a screenshot, performs OCR, gets an AI answer, and sends it as a .py file via Telegram.
    """
    ...
    try:
        ...
        caption = "Python code generated from screenhot OCR:"
...
def perform_selection_task():
    """
    Reads text from clipboard, gets an AI answer, and sends it as a .py file via Telegram.
    """
